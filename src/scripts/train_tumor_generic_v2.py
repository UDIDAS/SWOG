#!/usr/bin/env python3
"""Generic tumor model v2 — fine-tune the v1 (LiTS+Pancreas, 0.909) with the newly extracted FLARE
tumor slices folded in. Warm-start from v1, text='tumor', use_boxes=False, both GPUs. Per-dataset
autonomous eval afterward (FLARE tumor should jump from ~0.02)."""
import json
import os
import sys
...
POOLS = ["/scratch/ud3d4/acm_data/tumor_pool", "/scratch/ud3d4/acm_data/flare_tumor_pool"]
WARM = "/scratch/ud3d4/acm_data/tumor_pool/sam3_tumor_generic.pth"
CKPT = "/scratch/ud3d4/acm_data/tumor_pool/sam3_tumor_generic_v2.pth"
...
def autonomous_eval(X, Y, meta, te):
    ...
    print("\n===== v2 autonomous text-'tumor' Dice per dataset =====", flush=True)
