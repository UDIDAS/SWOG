#!/usr/bin/env python3
"""Run remaining FLARE sections (S3 fixed curriculum + S5 + S6) with visualization."""
import sys, os
...
def go():
    ...
    # ── S2: Evaluate existing baseline (with viz) ──
    s2_path = os.path.join(OUT, "s2_baseline_class12.pth")
    if os.path.exists(s2_path):
        print("\n" + "="*70)
        print("SECTION 2: Baseline Evaluation + Visualization")
        print("="*70)
        x_tr12, y_tr12, x_va12, y_va12, x_te12, y_te12 = load_and_split(12)
        te_c12 = get_or_generate_coords(y_te12, "test", 12)
        evaluate_test(s2_path, x_te12, y_te12, te_c12, viz_tag="s2_baseline_class12")
    ...
    print("\n=== S3 E2H Test ===")
    evaluate_test(s3e, x_te4, y_te4, te_c4, viz_tag="s3_e2h_class4")
    ...
    print("\n=== S3 H2E Test ===")
    evaluate_test(s3h, x_te4, y_te4, te_c4, viz_tag="s3_h2e_class4")
    ...
    # Also evaluate Liver on its own test set
    print("\n=== S5 Liver Test ===")
    evaluate_test(s5_liver, x_te1, y_te1, te_c1, viz_tag="s5_liver_class1")
    ...
    print("Visualizations saved to:", os.path.join(OUT, "viz/"))
    print("="*70)
