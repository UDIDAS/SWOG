...
| `union_ablation/` (see its README) | **OAKG vs OAKG-Union** | Observation-boundary ablation (all-111). **Null on aggregate ranking:** OAKG-Union ≥ OAKG every regime (asymmetric −0.088, dataset-style −0.044 Holm-sig; uniform +0.001 control) — OAKG abstains 27–38% in one-sided regimes. Mechanism value is in semantics + hard-distractor-vs-imputation, not aggregate nDCG |
| `native_cross_dataset/` (see its README) | **Native cross-dataset** | OAKG vs OAKG-Union under the *original unmasked* source scopes. Cross-source pooled +0.170 (Holm p=0.108, directional only); 31.9% of cross-source pairs incomparable natively. ✓ verified all-111-robust (OAKG served=1.0 every stratum; FLARE hub prevents full abstention) |
